...
.sg-item .sg-field{display:inline-block;padding:.1rem .45rem;font-size:.65rem;font-weight:600;background:rgba(16,185,129,.1);border-radius:10px;color:#059669;margin-left:auto;flex-shrink:0;text-transform:uppercase;letter-spacing:.03em}
...
.badge{display:inline-block;padding:.2rem .6rem;font-size:.7rem;font-weight:600;background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.2);border-radius:20px;color:#059669}
...
.modal-icon{display:inline-flex;align-items:center;justify-content:center;width:52px;height:52px;border-radius:50%;background:#10b981;margin-bottom:.75rem}
...
.modal-body .input-wrapper input,.modal-body .input-wrapper select{width:100%;padding:.7rem .85rem .7rem 2.75rem;font-family:inherit;font-size:.875rem;border:1.5px solid var(--g200);border-radius:var(--radius-sm);outline:none;transition:border-color var(--tr);-webkit-appearance:none;appearance:none}
...
.modal-footer .btn-reset:hover{border-color:var(--g400)}
...
@media(max-width:640px){.page-header{flex-direction:column;align-items:flex-start}.table-wrap{overflow-x:auto}table{min-width:700px}.modal-body .form-row{flex-direction:column;gap:0}.modal-body{padding:0 1.25rem}.modal-footer{flex-direction:column}.toolbar-btn{flex:1;justify-content:center;font-size:.72rem;padding:.45rem .6rem}}
</style>
@endsection

@section('content')
<div class="page-header">
    <div><h1>Teachers <span class="count-badge">{{ $teachers->total() }}</span></h1><p>Manage all teachers and staff.</p></div>
    <button type="button" class="btn-add" onclick="document.getElementById('add-teacher-modal').classList.add('active')">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Add Teacher
    </button>
</div>

<form action="{{ route('teachers.index') }}" method="GET" class="search-bar" id="search-form">
    <div class="search-input-wrap">
        ...
        <input type="text" name="search" id="search-input" value="{{ request('search') }}" placeholder="Search teachers…" autocomplete="off">
...
<div class="action-toolbar">
    <a href="{{ route('teachers.export.excel', ['search' => request('search')]) }}" class="toolbar-btn btn-excel" id="btn-export-excel">
    ...
    <a href="{{ route('teachers.export.pdf', ['search' => request('search')]) }}" class="toolbar-btn btn-pdf" id="btn-export-pdf">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="8" y1="13" x2="16" y2="13"/><line x1="8" y1="17" x2="16" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
        PDF
    </a>
...
<form action="{{ route('teachers.import') }}" method="POST" enctype="multipart/form-data" id="import-form" style="display:none">
    @csrf
...
@if($teachers->hasPages())<div style="margin-top:1.25rem;display:flex;justify-content:center">{{ $teachers->appends(request()->query())->links() }}</div>@endif

{{-- Add Teacher Modal --}}
<div class="modal-overlay" id="add-teacher-modal">
<div class="modal-container"><div class="modal-header">
<div class="modal-icon"><svg viewBox="0 0 24 24" fill="none" stroke="var(--white)" stroke-width="2"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div>
<h2>Add Teacher</h2><p>Enter teacher details below.</p>
...
<div class="form-group"><label>Email</label><div class="input-wrapper"><svg class="fi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M22 7l-10 7L2 7"/></svg><input type="email" name="email" placeholder="jane@school.com" required></div></div>
<div class="form-group"><label>Phone</label><div class="input-wrapper"><svg class="fi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6A19.79 19.79 0 012.12 4.18 2 2 0 014.11 2h3a2 2 0 012 1.72c.13.81.36 1.6.68 2.34a2 2 0 01-.45 2.11L8.09 9.41a16 16 0 006.5 6.5l1.24-1.24a2 2 0 012.11-.45c.74.32 1.53.55 2.34.68a2 2 0 011.72 2.02z"/></svg><input type="text" name="phone_number" placeholder="0779998888" pattern="^(0[0-9]{9}|[1-9][0-9]{8})$" title="Phone number must be 10 digits if starting with 0, or 9 digits otherwise." required></div></div>
<div class="form-group"><label>Subject</label><div class="input-wrapper sw"><svg class="fi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 19.5A2.5 2.5 0 016.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z"/></svg><select name="subject" required><option value="" disabled selected>Select subject…</option><option>Computer Science</option><option>Mathematics</option><option>Physics</option><option>English</option><option>Data Science</option><option>Cyber Security</option></select></div></div>
</div>
...
<button type="submit" class="btn-submit"><span>Add Teacher</span><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg></button>
</div></form></div></div>
...
<h2>Edit Teacher</h2><p>Update teacher information.</p>
<button type="button" class="modal-close" onclick="closeModal('edit-teacher-modal')">&times;</button></div>
...
<div class="form-group"><label>Email</label><div class="input-wrapper"><svg class="fi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M22 7l-10 7L2 7"/></svg><input type="email" name="email" id="et_em" required></div></div>
<div class="form-group"><label>Phone</label><div class="input-wrapper"><svg class="fi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6A19.79 19.79 0 012.12 4.18 2 2 0 014.11 2h3a2 2 0 012 1.72c.13.81.36 1.6.68 2.34a2 2 0 01-.45 2.11L8.09 9.41a16 16 0 006.5 6.5l1.24-1.24a2 2 0 012.11-.45c.74.32 1.53.55 2.34.68a2 2 0 011.72 2.02z"/></svg><input type="text" name="phone_number" id="et_ph" pattern="^(0[0-9]{9}|[1-9][0-9]{8})$" title="Phone number must be 10 digits if starting with 0, or 9 digits otherwise." required></div></div>
<div class="form-group"><label>Subject</label><div class="input-wrapper sw"><svg class="fi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 19.5A2.5 2.5 0 016.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z"/></svg><select name="subject" id="et_sub" required><option value="" disabled>Select subject…</option><option>Computer Science</option><option>Mathematics</option><option>Physics</option><option>English</option><option>Data Science</option><option>Cyber Security</option></select></div></div>
</div>
<div class="modal-footer">
<button type="button" class="btn-reset" onclick="closeModal('edit-teacher-modal')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18"/><path d="M6 6l12 12"/></svg>Cancel</button>
<button type="submit" class="btn-submit"><span>Update Teacher</span><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg></button>
</div></form></div></div>
@endsection
...
if(window.location.hash==='#add')document.getElementById('add-teacher-modal').classList.add('active');
(function(){
    var si=document.getElementById('search-input'),cb=document.getElementById('btn-clear'),tb=document.querySelector('#teachers-table tbody'),sf=document.getElementById('search-form'),dd=document.getElementById('suggestions-dropdown');
    ...
    cb.addEventListener('click',function(){si.value='';tc();closeSg();if(new URLSearchParams(window.location.search).has('search'))window.location.href='{{ route("teachers.index") }}';else if(tb)tb.querySelectorAll('tr').forEach(function(r){r.style.display=''});si.focus()});
    ...
    function fetchSg(q){
        ...
        fetch('{{ route("teachers.search") }}?q='+encodeURIComponent(q),{headers:{'X-Requested-With':'XMLHttpRequest','Accept':'application/json'}})
        .then(function(r){return r.json()}).then(function(data){
            ...
            dd.innerHTML=data.map(function(item){
                return '<div class="sg-item" data-search="'+esc(item.search_value)+'"><div class="sg-icon"><svg viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div><div class="sg-info"><div class="sg-name">'+esc(item.name)+'</div><div class="sg-meta">'+esc(item.detail)+'</div></div><span class="sg-field">'+esc(item.matched_field)+'</span></div>';
            }).join('');
...
document.querySelectorAll('.delete-form').forEach(function(f){f.addEventListener('submit',function(e){e.preventDefault();var df=this;Swal.fire({title:'Delete Teacher?',text:'This cannot be undone.',icon:'warning',showCancelButton:true,confirmButtonColor:'#0a0a0a',confirmButtonText:'Yes, delete!'}).then(function(r){if(r.isConfirmed)df.submit()})})});
document.getElementById('add-teacher-form').addEventListener('submit',function(e){e.preventDefault();var f=this;Swal.fire({title:'Add Teacher?',icon:'question',showCancelButton:true,confirmButtonColor:'#0a0a0a',confirmButtonText:'Yes, add!'}).then(function(r){if(r.isConfirmed)f.submit()})});
document.getElementById('edit-teacher-form').addEventListener('submit',function(e){e.preventDefault();var f=this;Swal.fire({title:'Update Teacher?',icon:'question',showCancelButton:true,confirmButtonColor:'#0a0a0a',confirmButtonText:'Yes, update!'}).then(function(r){if(r.isConfirmed)f.submit()})});
...
function openImportModal(){
    Swal.fire({
        ...
        html:'<div style="text-align:left;font-size:.85rem;color:#525252;margin-bottom:1rem">' +
             ...
             'First Name, Last Name, Email, Phone Number, Subject</div>' +
             '<p style="font-size:.78rem;color:#a3a3a3">• First row should be headers<br>• Duplicate emails will be skipped<br>• Max file size: 10MB</p></div>' +
...
(function(){
    ...
    var baseExcel='{{ route("teachers.export.excel") }}';
    var basePdf='{{ route("teachers.export.pdf") }}';
